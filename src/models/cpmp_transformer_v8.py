...
class CPMPTransformer(Transformer):
    def __init__(self, H, C_dim, X_dim, d_model=64, nhead=8, num_layers=2, ff_dim_multiplier=4, dropout=0.1):
        ...
        self.cost_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.LayerNorm(d_model), # Estabiliza el entrenamiento
            nn.Linear(d_model, 1)
        )
    ...
    def forward(self, S, X):
        ...
        attn_weights = torch.softmax(self.cost_attention(x_global), dim=1)
        x_for_cost = torch.sum(x_global * attn_weights, dim=1)
