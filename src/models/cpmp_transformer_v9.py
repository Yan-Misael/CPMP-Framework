...
class CPMPTransformer(Transformer):
    def __init__(self, H, C_dim, X_dim, d_model=64, nhead=8, num_layers=2, ff_dim_multiplier=4, dropout=0.1):
        ...
        # --- BLOQUE DE POLÍTICA (Para Logits/Acciones) ---
        ...
        # --- BLOQUE DE VALOR (Para Costo) ---
        # Puede tener menos capas si quieres ahorrar tiempo, o las mismas para máxima capacidad
        ...
        self.cost_head = nn.Sequential(
            nn.Linear(d_model * 2, d_model * ff_dim_multiplier), # *2 por el cat de Max + Attn
            nn.GELU(),
    ...
    def forward(self, S, X):
        ...
        # 2. Rama de Política (Gradiente fluye aquí para acciones)
        x_policy = self.policy_inter_stack(stack_embeddings)
        ...
        # 3. Rama de Valor (Aplicamos DETACH aquí para proteger la política)
        # Detachamos los embeddings base para que el costo no cambie el procesamiento de stacks
        x_value_base = stack_embeddings.detach() 
        
        x_value = self.value_inter_stack(x_value_base)
        
        attn_weights = torch.softmax(self.cost_attention(x_value), dim=1)
        x_for_cost = torch.sum(x_value * attn_weights, dim=1)
